...
# Imports that might take time or log
from app.api.endpoints import analysis, health, docs_endpoints
...
async def lifespan(app: FastAPI):
    ...
    # Load ML models explicitly during startup
    from app.services.ml_pipeline import ml_pipeline
    logger.info("🔥 Triggering ML model loading...")
    
    # This might take time, but logging is ready now
...
app = FastAPI(
    ...
    description="Knowledge-enhanced multimodal misinformation detection (English-only)",
    version="1.0.0",
    lifespan=lifespan
)
